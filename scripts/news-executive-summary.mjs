...
export function normalizeExecutiveSummary(value) {
  ...
  const headline = stringValue(value.headline, 120);
  const overview = stringValue(value.overview, 700);
  const pulseLabel = stringValue(value.pulse?.label, 32);
  const pulseSummary = stringValue(value.pulse?.summary, 400);
  if (
  ...
  const bullets = value.bullets.flatMap((bullet) => {
    ...
    const bulletHeadline = stringValue(bullet.headline, 110);
    const summary = stringValue(bullet.summary, 360);
    const whyItMatters = stringValue(bullet.whyItMatters, 420);
    const details = stringValue(bullet.details, 900);
    const sources = Array.isArray(bullet.sources)
      ? bullet.sources.map(normalizeSourceReference).filter(Boolean).slice(0, 6)
      : [];
    if (!bulletHeadline || !summary || !whyItMatters || !details || sources.length === 0) return [];
    return [{ headline: bulletHeadline, summary, whyItMatters, details, sources }];
  }).slice(0, 7);
  if (bullets.length < 3) return undefined;

  const watchNext = Array.isArray(value.watchNext)
    ? value.watchNext.flatMap((entry) => stringValue(entry, 240) ?? []).slice(0, 5)
    : [];
  const noiseSummary = stringValue(value.noiseSummary, 400);
...
function promptFor(items, previousSummary, now) {
  ...
  return [
    ...
    'Write a clear, crisp executive pulse: what is happening, what matters, why it matters, and what to watch next.',
    'The collapsed summary field for every bullet must stand alone. Put nuance and evidence in details.',
